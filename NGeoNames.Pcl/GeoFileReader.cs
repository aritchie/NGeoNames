...
namespace NGeoNames
{
    ...
    public class GeoFileReader
    {
        ...
        //{
        //    return ReadRecords(path, parser);
        //}
        ...
        {
            return FileSystem.Instance.GetFile(path).OpenRead();
        ...
        static readonly ResourceManager resourceManager = new ResourceManager(typeof(GeoFileReader));
        private static IEnumerable<T> ReadBuiltInResource<T>(string name, IParser<T> parser)
        {
            using (var s = new MemoryStream(parser.Encoding.GetBytes(resourceManager.GetString(name))))
