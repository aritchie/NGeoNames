...
namespace NGeoNames.Tests
{
    internal class FileUtil
    {
        ...
        public static void EnsureFilesAreFunctionallyEqual(string src, string dst, int expectedfields, int skiplines, char[] fieldseparators, Encoding encoding, bool hascomments)
        {
            ...
            var expected = new GeoFileReader().ReadRecords<GenericEntity>(src, parser_in).ToArray();
            var actual = new GeoFileReader().ReadRecords<GenericEntity>(dst, parser_out).ToArray();
